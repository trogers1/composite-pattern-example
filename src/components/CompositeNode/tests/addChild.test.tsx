...
describe('CompositeNode', () => {
  ...
  describe('addChild: CompositeNode', () => {
    it('should add a child to a potential parent node when appropriate', () => {
      ...
      const potentialChild: CompositeNode = new CompositeNode({ id: '343', name: 'test343', parent: null });
      const rootResult = parent.addChild({ node: potentialChild });
      expect(rootResult).toStrictEqual(parent);
      const expectedChildFormat = new CompositeNode({
    ...
    it('should NOT the a child of the same ID twice (nor will it overwrite the child)', () => {
      ...
      const potentialChild: CompositeNode = new CompositeNode({ id: '343', name: 'test343', parent });
      const potentialChild2: CompositeNode = new CompositeNode({
        id: '343',
        name: 'test343.2',
        parent: potentialChild,
      });
      let rootResult = parent.addChild({ node: potentialChild });
      expect(rootResult).toStrictEqual(parent);
      rootResult = parent.addChild({ node: potentialChild2 });
      expect(rootResult).toStrictEqual(null);
      const expectedChildFormat = new CompositeNode({
        id: potentialChild.getId(),
        name: potentialChild.getName(),
    ...
    it('should add MULTIPLE children to a potential parent node when appropriate', () => {
      ...
      const potentialChild: CompositeNode = new CompositeNode({ id: '343', name: 'test343', parent: null });
      const potentialChild2: CompositeNode = new CompositeNode({ id: '344', name: 'test344', parent: null });
      ...
      expect(rootResult).toStrictEqual(parent);
      const expectedChildFormat2 = new CompositeNode({
        id: potentialChild2.getId(),
        name: potentialChild2.getName(),
        parent,
        onChange: (args) => {},
      });
      expect(() => {
    ...
    it('should return `null` if the two nodes are unrelated', () => {
      ...
      const potentialChild: CompositeNode = new CompositeNode({ id: '21', name: 'test2', parent });
      const rootResult = parent.addChild({ node: potentialChild });
      expect(rootResult).toStrictEqual(null);
      // Make sure no children were added and that the node remains unmolested
      checkNodeProperties({ node: parent, isSelected: -1, parent: null, children: [], id: '1', name: 'test1' });
    ...
    it('should create a new Node (adding the initial node as a child) and return it as the root if the potential child should, in fact, be the parent node through MULTIPLE generations', () => {
      ...
      const potentialChild: CompositeNode = new CompositeNode({ id: '439', name: 'test439', parent: null });
      const potentialChild2: CompositeNode = new CompositeNode({ id: '43', name: 'test43', parent: null });
      let rootResult = initialNode.addChild({ node: potentialChild });
      rootResult = initialNode.addChild({ node: potentialChild2 });
      expect(rootResult).toBeInstanceOf(CompositeNode);
      // Check that the end rootResult is potentialChild2 at the top level
      checkNodeProperties({
        node: rootResult!,
    ...
    it('should add a new node in the middle of a tree, adopting a single child', () => {
      ...
      const node4 = new CompositeNode({ id: '4', name: 'test4', parent: null, children: [node439] });
      expect(node4.getChildren()).toStrictEqual([node439]);
      const node43: CompositeNode = new CompositeNode({ id: '43', name: 'test43', parent: node4391 });
      const root = node439.addChild({ node: node43 });

      expect(root).toStrictEqual(node4);
      expect(root!.getChildren()).toStrictEqual([
        new CompositeNode({ id: node43.getId(), name: node43.getName(), children: [node439], parent: node4 }),
      ]);
    });
    it('should add a new node in the middle of a tree, adopting multiple children', () => {
      const node4391 = new CompositeNode({ id: '4391', name: 'test4391', parent: null });
